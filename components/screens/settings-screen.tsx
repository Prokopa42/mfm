...
function FullResetBlock({ onOpen }: { onOpen: () => void }) {
  return (
    <Group title="Полный сброс" note="очистка">
      ...
      >
        Отдельно от «Старт работы». Полностью обнуляет деньги, дневник, платежи, накопления, цели и долговые обязательства.
      ...
      <DialogContent>
        ...
        <DialogBody>
          <div className="mono" style={{ fontSize: 9.5, lineHeight: 1.5, color: "var(--ink-70)" }}>
            ...
            снятия, обязательные платежи, долговые обязательства и долговые движения.
          </div>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Обнуляем не деньги, а журнал. Остатки, цели, рубрики и долговые обязательства остаются как новая стартовая точка.
          </DialogDescription>
        </DialogHeader>
        <DialogBody style={{ gap: 12, maxHeight: "68vh", overflow: "auto" }}>
          <GoLiveList
            ...
            items={[
              ...
              "рубрики",
              "долговые обязательства с текущими остатками"
            ]}
          />
          <GoLiveList
            title="Очистим"
            ...
            <div key={scope} style={{ borderTop: "1px solid var(--ink)" }}>
              ...
              {scope === "mandatory-payment" && (
                <div className="mono" style={{ padding: "0 0 5px", fontSize: 8.8, color: "var(--ink-55)", lineHeight: 1.4 }}>
                  Эти рубрики используются при создании платежей на вкладке Цикл: аренда, связь, долги, подписки и т.д.
                </div>
              )}
